...
public class TicketDAO {
    ...
    public static ArrayList<Ticket> getListTicketByUserId(int userid) {
        ...
        try {
            ...
            String query = "SELECT ticket.id as id, vehicle.plate as plate, park.park_code as parkCode, ticket.ticket_code as ticketCode, "
    ...
    public static ArrayList<Ticket> getListTicketByParkId(int parkid) {
        ...
        try {
            DbConnect connect = new DbConnect();
            String query = "SELECT t.id, v.plate, p.park_code, t.ticket_code, t.status, t.fee, t.checkin_time, t.checkout_time "
                    + "FROM ticket t, park p, vehicle v " + "WHERE p.id = t.park_id and v.id = t.vehicle_id "
                    + "and t.checkin_time > curdate() " + "and p.id = ? order by status desc, checkin_time desc ;";
    ...
    public static Ticket getTicketByPlate(String plate) {
        ...
        try {
            DbConnect connect = new DbConnect();
            String query = "SELECT t.id, t.status, t.checkin_time, t.ticket_code, v.plate "
